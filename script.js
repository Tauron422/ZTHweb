...
const Config = {
    smartContractId: "738377637144987047",
...
function calculateBuyFromSigna(Signa) {
    ...
    let remPrice = (Number(Stats.signaTotal + effSigna) / 1E8) / (Number(Stats.assetTotal - effAsset) / 1)

    const poolFeeSigna = Number(bSigna - effSigna) / 1E8
    const impact = (remPrice - Stats.aPrice) / Stats.aPrice
    const effectivePrice = (Signa) / (Number(effAsset) / 1)
    const effectiveAsset = Number(effAsset) / 1
...
function calculateBuyFromTmg(Tmg) {
    ...
    let bTmg = BigInt((Tmg * 1).toFixed(0))
    ...
    let remPrice = (Number(Stats.signaTotal + (bSigna*980n)/1000n) / 1E8) / (Number(Stats.assetTotal - bTmg) / 1)
    ...
    const impact = (remPrice - Stats.aPrice) / Stats.aPrice
    const effectivePrice = (neededSigna) / (Number(bTmg) / 1)
...
function calculateAdd(Signa, Asset) {
    ...
    let bAsset = BigInt((Asset * 1).toFixed(0))
    ...
    const refundedAsset = Number(excessAsset) / 1
...
function calculateRemove(Asset) {
    ...
    const removedAsset = Number(calculatedAsset) / 1
...
function calculateSellFromTMG(Asset) {
    ...
    const poolFeeTmg = Number(bAsset - effAsset) / 1
    const impact = (Stats.aPrice - remPrice) / Stats.aPrice
    const effectivePrice = ((Number(effSigna - Config.contractActivation - Config.transactionFee))/1E8) / (Number(bAsset) / 1)
...
function calculateSellFromSigna(Signa) {
    ...
    let remPrice = (Number(Stats.signaTotal - bSigna) / 1E8) / (Number(Stats.assetTotal + effectiveAsset) / 1)

    const poolFeeTmg = Number(assetTotal - effectiveAsset) / 1
    const impact = (Stats.aPrice - remPrice) / Stats.aPrice
    const effectivePrice = (Number(bSigna - Config.contractActivation - Config.transactionFee)/1E8) / (Number(assetTotal) / 1)
    const neededAsset = Number(assetTotal) / 1
...
function evtCalculateAddTmg(e) {
    ...
    const bTmg = BigInt((numberTmg * 1).toFixed(0))
...
function evtBuySigna(e) {
    ...
    document.getElementById('buy_price_impact').innerText = (Params.impact * 1).toFixed(2)
...
function evtSellTmg(e) {
    ...
    document.getElementById('sell_price_impact').innerText = (Params.impact * 1).toFixed(2)
...
async function evtBuy() {
    ...
    const parameters = {
        ...
        feeNQT: Config.transactionFee.toString(1),
        message: "trade",
        messageIsText: "true",
        deadline: "8",
    }
...
async function evtSell() {
    ...
    const parameters = {
        ...
        quantityQNT: (numberBalance*1).toFixed(0),
        amountNQT: Config.contractActivation.toString(1),
        publicKey: Global.walletResponse.publicKey,
        recipient: Config.smartContractId,
        message: "trade",
        messageIsText: "true",
...
async function evtAdd() {
    ...
    const parameters = {
        asset: Config.assetId,
        quantityQNT: (numberTmg*1).toFixed(0),
...
async function getExtendedAccountInfo() {
    ...
    if (Response.assetBalances === undefined) {
        ...
        if (tmgUB) {
            Global.extendedInfo.tmg = Number(tmgUB.unconfirmedBalanceQNT) / 1
        } else {
            Global.extendedInfo.tmg = 0
        }
        const tmg = Response.assetBalances.find(Obj => Obj.asset === Config.assetId)
        if (tmg) {
            Global.extendedInfo.tmgLocked = (Number(tmg.balanceQNT) / 1) - Global.extendedInfo.tmg
        } else {
            Global.extendedInfo.tmgLocked = 0
        }
        const lctmgUB = Response.unconfirmedAssetBalances.find(Obj => Obj.asset === Config.lcId)
        if (lctmgUB) {
            Global.extendedInfo.lctmg = Number(lctmgUB.unconfirmedBalanceQNT) / 10
        } else {
            Global.extendedInfo.lctmg = 0
        }
        const lctmg = Response.assetBalances.find(Obj => Obj.asset === Config.lcId)
        if (lctmg) {
            Global.extendedInfo.lctmgLocked = (Number(lctmg.balanceQNT) / 10) - Global.extendedInfo.lctmg
        } else {
            Global.extendedInfo.lctmgLocked = 0
        }
    }
...
function updateContractDetails() {
    ...
    document.getElementById("contract_tmg").innerText = Number(Stats.assetTotal)/1
...
async function getBuyOrders() {
    ...
    for (const Order of OrdersResp.bidOrders) {
        ...
        output += `<td>${Number(Order.quantityQNT)/1}</td>`
        output += `<td><strong>${Number(Order.priceNQT)/100000}</strong></td>`
        output += `<td>${((Number(Order.priceNQT)*Number(Order.quantityQNT))/10000000).toFixed(2)}</td>`
        output += '</tr>'
    }
    ...
    for (const tx of transactions) {
        ...
        case 'trade':
            if (tx.amountNQT === '42000000') {
                action = 'Sell'
                amount = (Number(tx.attachment.quantityQNT)/1).toString() + " TMG"
            } else {
                action = 'Buy TMG'
        ...
        case 'add':
            ...
            amount = (Number(tx.attachment.quantityQNT)/1).toString() + " TMG + " + ((Number(tx.amountNQT)/1E8)-0.42).toFixed(0) + " SIGNA"
            break;
        case 'remove':
